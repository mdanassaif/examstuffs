...
export function MainPage() {
  ...
  const handleSearch = async (newStart = 1) => {
    ...
    let apiUrl = "";
    if (filter === "video") {
      apiUrl = `https://www.googleapis.com/youtube/v3/search?key=${youtubeApiKey}&q=${encodeURIComponent(
        query
      )}&part=snippet&type=video&maxResults=${itemsPerPage}&pageToken=${newStart}`;
      console.log(apiUrl);
    } else {
      apiUrl =
        googleSearchUrl +
            ...
            <div key={index} className="mb-4 p-4 border rounded">
              ...
              >
                <h2 className="text-xl font-bold">
                  {filter === "video" ? result.snippet?.title : result.title}
                </h2>
              </a>
